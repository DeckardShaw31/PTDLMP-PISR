...
1. **RQ1 (Predictability of Promised Delivery Lateness Proxy)**:
   ...
   - *Label Definition*: The public Amazon Challenge dataset does not provide observed customer-level delivery completion timestamps. Consequently, ground-truth delivery lateness is strictly defined as a **route-propagated promised-time violation proxy** computed by propagating the driver's actual sequence through the official historical average travel-time matrix and planned service durations. Base lateness prevalence is 50.34%.
    - *Model Protocol & Calibration Fit Diagnostics*: Logistic Regression was predeclared as the primary probability model. The validation block was used exclusively for Platt calibration, and predictive performance was evaluated on the held-out test block. Validation metrics serve as calibration-fit diagnostics (Logistic Regression achieved validation Brier score = **0.2387**, validation log loss = **0.6640**, validation ECE = **0.0159**, and validation ROC-AUC = **0.5327**). Out-of-sample evaluation on the held-out test block yielded ROC-AUC = 0.595, PR-AUC = 0.596, and Brier score = 0.2404.
...
3. **Core Hypothesis Resolution & Defensible Statistical Verdict**:
   - **Defensible Conclusion**: Actionability dominance ($g_i = \max \Delta TT$) drives simulated tardiness reductions; pure risk without actionability ($RB$) fails. No evidence of a difference between RA and AB was detected. A nonsignificant difference does not establish equivalence; establishing equivalence requires a prespecified practical margin and a formal Two One-Sided Tests (TOST) or confidence-interval equivalence procedure on an expanded cohort.
   - **Route-Clustered Statistical Inference ($N=3$)**:
     ...
     - **RA vs. AB (Pure Actionability)**: Mean diff = **-16.62 min**, route-level 95% bootstrap CI = **[-55.48, +3.95] min**, unadjusted $p = 0.4825$, Holm-Bonferroni adjusted $p = 0.4825$. No evidence of a difference between RA and AB was detected.
...
- **Chronological Split**:
  ...
  - **Validation (Platt Calibration Fit)**: 331 stops across next 2 dates (20%).
...
3. **Distinction Between Lack of Difference and Equivalence (RA vs. AB)**: Both at the cell level ($p = 0.3626$) and route-level aggregated paired test ($p = 0.4825$), no evidence of a difference between RA and AB was detected. A nonsignificant difference does not establish equivalence. A future equivalence analysis requires a prespecified practical margin and a formal Two One-Sided Tests (TOST) or confidence-interval equivalence test on an expanded cohort. The present result confirms that actionability ($g_i$) drives the observed simulated improvement, while risk weighting has not yet demonstrated incremental benefit over AB.
