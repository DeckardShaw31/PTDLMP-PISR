...
\section{\texorpdfstring{\textbf{Prediction-Informed Selective Route
Resequencing for Promised-Time Delivery Lateness Mitigation under
Limited Intervention
Capacity}}{Prediction-Informed Selective Route Resequencing for Promised-Time Delivery Lateness Mitigation under Limited Intervention Capacity}}\label{prediction-informed-selective-route-resequencing-for-promised-time-delivery-lateness-mitigation-under-limited-intervention-capacity}
...
\begin{quote}
\emph{\textbf{Keywords:}} Last-mile delivery; Delivery lateness
prediction; Prediction-informed decision making; Route resequencing;
Service reliability
\end{quote}

\textbf{\hl{LINK DATASET:}}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \href{https://www.kaggle.com/datasets/hunhquanglc/xe-d-ho-chi-minh-city-delivery-service-dataset}{\textbf{\hl{https://www.kaggle.com/datasets/hunhquanglc/xe-d-ho-chi-minh-city-delivery-service-dataset}}}
\item
  \href{https://registry.opendata.aws/amazon-last-mile-challenges/}{\textbf{\hl{https://registry.opendata.aws/amazon-last-mile-challenges/}}}
\item
  \href{https://cainiaoai.github.io/LaDe-website/}{\textbf{\hl{https://cainiaoai.github.io/LaDe-website/}}}
\end{enumerate}
...
\section{\texorpdfstring{\textbf{2. Literature
...
\textbf{2.1 Delivery lateness prediction in e-commerce and last-mile
logistics}
...
\textbf{2.2 Prediction-informed decision making in logistics}
...
drivers\textquotesingle{} route trajectories, while Canoy et al. (2024)
...
\textbf{2.3 Route adjustment and service reliability in last-mile
delivery}
...
changes not only that customer\textquotesingle s arrival time but
...
\subsection{\texorpdfstring{\textbf{3. Prediction-Informed Selective
Route Resequencing (PISR)
Framework}}{3. Prediction-Informed Selective Route Resequencing (PISR) Framework}}\label{prediction-informed-selective-route-resequencing-pisr-framework}

\textbf{3.1 Problem definition and decision setting}
...
\textbf{Definition 1 (Promised-time lateness).} Customer \(i\) is late
...
\textbf{Table 1.} Used notations in the proposed method
...
\bottomrule\noalign{}
\endlastfoot
\(C\) & Set of customers assigned to a route, indexed by \(i\ \) &
\(T_{i}(R)\) & Tardiness of customer \(i\) under route \(R\) \\
\(n\) & Number of customers & \(NL(R)\) & Number of late deliveries
under route \(R\) \\
\(r\) & Courier-day route index & \(TT(R)\) & Total route tardiness
under route \(R\) \\
\(n_{r}\) & Number of customers assigned to courier-day route \(r\) &
\(p_{i}\) & Predicted promised-time lateness probability of customer
\(i\) \\
\(R\) & Generic customer service sequence & \(g_{i}\) & Intervention
actionability score of customer \(i\) \\
\(R^{0}\) & Ex-ante baseline customer service sequence &
\(\Delta TT_{ij}\) & Potential reduction in baseline total tardiness
from relocating customer \(i\) to position \(j\) \\
\(R^{c}\) & Current customer service sequence during sequential
intervention & \(F_{i}\) & Set of feasible forward positions for
customer \(i\) under the actionability assessment \\
\(R'\) & Final customer service sequence after intervention & \(J_{i}\)
& Set of candidate forward positions for customer \(i\) \\
\(R^{0,i \rightarrow j}\) & Route obtained by relocating customer \(i\)
from its position in baseline route \(R^{0}\) to position \(j\) &
\(S_{i}^{RB}\) & Risk-Based targeting score of customer \(i\) \\
\(R^{c,i \rightarrow j}\) & Route obtained by relocating customer \(i\)
from its position in route \(R^{C}\) to position \(j\) & \(S_{i}^{AB}\)
& Actionability-Based targeting score of customer \(i\) \\
\(i_{k}\) & Customer occupying position \(k\) in a service sequence &
\(RAS_{i}\) & Risk-Actionability score of customer \(i\) \\
\(j\) & Candidate insertion position for a forward relocation & \(B\) &
Intervention-budget proportion \\
\(k\) & Current position of the selected customer in a service sequence
& \(K_{r}(B)\) & Maximum number of customers admitted for intervention
on route \(r\) under budget \(B\) \\
\(\tau_{i}\) & Promised delivery time of customer \(i\) & \(C_{r}^{B}\)
& Set of customers admitted for intervention on route \(r\) under budget
\(B\) \\
\(d_{h}\) & Realized delivery time of historical order \(h\) &
\(\delta\) & Maximum allowable relative increase in route distance \\
\(y_{h}\) & Observed promised-time lateness label of historical order
\(h\) & \(D(R)\) & Total route distance \(R\) \\
\(x_{i}\) & Ex-ante predictor vector of customer \(i\) & \(\Delta NL\) &
Change in number of late deliveries between baseline route \(R^{0}\) and
final route \(R'\) \\
\(a_{i}(R)\) & Route-propagated service time of customer \(i\) under
route \(R\) & \(\Delta TT\) & Reduction in total tardiness between
baseline route \(R^{0}\) and final route \(R'\) \\
\(L_{i}(R)\) & Promised-time lateness indicator of customer \(i\) under
route \(R\) & \(\Delta D\) & Relative change in route distance between
baseline route \(R^{0}\) and final route \(R'\) \\
\end{longtable}
}

\textbf{3.2 Framework architecture and decision flow}
...
\textbf{Figure 1.} Prediction-Informed Selective Route Resequencing
framework

\textbf{3.3. Ex-ante lateness-risk estimation}
...
\includegraphics[width=6.5in,height=3.25in]{media/media/image1.png}observations
...
changing the customer\textquotesingle s route position offers meaningful
operational potential. Moreover, \(p_{i}\) does not enter the local
route-improvement objective.

\textbf{3.4. Ex-ante baseline route construction}
...
\textbf{Condition 1 (Ex-ante baseline-route construction).} The baseline
route \(R^{0}\) must be constructed without using realized delivery
timestamps, the realized customer service sequence, or other
...
\textbf{3.5. Risk- and actionability-informed intervention
prioritization}
...
\textbf{Definition 2 (Risk--Actionability Score).} \emph{Under the
...
\textbf{3.6 Selective forward relocation under routing constraints}
...
\textbf{Algorithm 1. Selective Forward Relocation (SFR)}

\textbf{Input:} Baseline route \(R^{0}\), ranked intervention candidate
set \(C_{r}^{B}\), route-distance tolerance \(\delta\)\\
\textbf{Output:} Final route \(R'\)

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Set \(R^{c} \leftarrow R^{0}\).
\item
  \textbf{For} each \(i \in C_{r}^{B}\), following the predetermined in
  descending targeting-score order \textbf{do}
\item
  Identify the current position \(k\) of customer \(i\) in \(R^{c}\);
\item
  Enumerate candidate positions \(j < k\);
\item
  Construct \(R^{c,i \rightarrow j}\) for each candidate position;
\item
  Retain candidates satisfying Eq. (15);
\item
  \textbf{If} no admissible candidate exists \textbf{then}
\item
  Continue to the next customer;
\item
  \textbf{End If}
\item
  Select \(j^{*}\) according to Eq. (16);
\item
  \textbf{If} Eq. (17) holds \textbf{then}
\item
  Set \(R^{c} \leftarrow R^{c,i \rightarrow j^{*}}\).
\item
  \textbf{End If}
\item
  \textbf{End For}
\item
  Return \(R' \leftarrow R^{c}\).
\end{enumerate}

\textbf{Proposition 1 (Monotonicity of total tardiness under SFR).}
\emph{For baseline route} \(R^{0}\) \emph{and final route} \(R'\)
...
\textbf{Proof.} Each accepted relocation satisfies Eq. (17) and
...
inequality whenever at least one relocation is accepted. \(\square\)
...
\textbf{3.7 Counterfactual route outcome evaluation}
...
\textbf{Definition 3 (Counterfactual route outcome).} \emph{A
counterfactual route outcome is obtained by applying the same route-time
propagation procedure to an alternative service sequence while holding
the applicable non-routing inputs fixed. Lateness and tardiness are
therefore recomputed from the resulting route rather than assigned
according to whether a customer was selected or relocated.}
...
mechanically change a customer\textquotesingle s lateness status. This
