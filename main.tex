...
\section{Empirical Evaluation and Numerical Experiments}\label{sec:empirical-evaluation}

\subsection{Experimental Data and Operational Cohort}\label{sec:data-cohort}
We evaluate the PISR framework on dispatch routes from the official Amazon Last Mile Routing Research Challenge dataset (AWS Open Data, licensed under CC BY-NC 4.0). The empirical cohort analyzed in this study comprises 13 dispatch routes spanning 2,051 total stops (2,038 drop-off delivery stops) and 3,129 packages across 12 calendar dates from 7 distribution centers (\texttt{DBO2}, \texttt{DCH4}, \texttt{DLA7}, \texttt{DLA8}, \texttt{DLA9}, \texttt{DSE4}, and \texttt{DSE5}).

To maintain strict empirical rigor:
\begin{enumerate}
    \item \textbf{Target Label Definition}: The public Amazon Challenge dataset does not provide realized customer-level delivery completion timestamps. Consequently, ground-truth delivery lateness is strictly defined as a \emph{route-propagated promised-time violation proxy} computed by propagating the driver's actual sequence through the official historical average travel-time matrix and planned service durations.
    \item \textbf{Distance Metric}: All route distances $D(R)$ and distance increments $\Delta D$ are evaluated using a \emph{Haversine straight-line distance proxy} between customer coordinates, maintaining deterministic $O(1)$ local relocation evaluation.
    \item \textbf{Strict SLA Constraints}: The experimental pipeline enforces \texttt{default\_sla\_hours=None}, ensuring that zero synthetic deadlines are imputed. Only packages with explicit customer promised delivery times are treated as deadline-constrained.
    \item \textbf{Zero Imputed Travel Times}: All travel times between stops are retrieved strictly from official historical travel-time matrices without artificial distance-to-time edge imputation.
\end{enumerate}
...
To reflect realistic dispatch conditions and prevent temporal data leakage, observations are partitioned chronologically by calendar date:
\begin{itemize}
    \item \textbf{Training Block}: Earliest 7 dates (60\% of observations, 1,173 stops).
    \item \textbf{Validation Block}: Intermediate 2 dates (20\% of observations, 331 stops).
    \item \textbf{Held-Out Test Block}: Latest 3 dates (20\% of observations, 534 stops).
\end{itemize}
A strict feature leakage guard restricts model inputs to ex-ante characteristics known prior to vehicle departure ($t \le \text{departure\_time}$): depot Haversine distance, local 2~km stop density, planned service duration, package volume, stop package count, route-level stop count, planned service minutes, departure hour, day of week, and deadline slack ($\tau_i - \text{departure\_time}$).

\paragraph{Predeclared Probability Model and Calibration-Fit Diagnostics.}
Logistic Regression is predeclared as the primary probability model architecture. To avoid optimistic model selection on validation data, the chronological validation block is reserved exclusively for fitting post-hoc Platt scaling (probability calibration). Predictive performance is evaluated out-of-sample on the held-out test block. Validation metrics serve as calibration-fit diagnostics rather than model-selection evidence.

\subsection{Empirical Routing Benchmark Design (RQ2)}\label{sec:rq2-design}
The routing benchmark addresses RQ2 by evaluating whether selective forward resequencing mitigates total route tardiness without violating operational capacity or route distance limits.

\paragraph{Factorial Evaluation Grid.}
Experiments are conducted across 288 factorial runs on the 3 held-out test routes (\texttt{DBO2}, \texttt{DSE4}, and \texttt{DSE5}) spanning:
\begin{itemize}
    \item Intervention budgets: $B \in \{5\%, 10\%, 20\%, 30\%\}$.
    \item Cumulative route-distance tolerances: $\delta \in \{0\%, 2\%, 5\%, 10\%\}$.
\end{itemize}
Five targeting policies are compared: Risk-Actionability (RA: $p_i g_i$), Actionability-Based (AB: $g_i$), Risk-Based (RB: $p_i$), Slack-Based, and Deadline-Based, alongside a Uniform Random baseline evaluated across 10 distinct random seeds (seeds 42 to 51) per parameter cell.

\paragraph{Baseline Routing Heuristics.}
To examine heuristic robustness, baseline routes $R^0$ are constructed using:
\begin{enumerate}
...
\paragraph{Invariant Verification and Hard Feasibility.}
Every generated route $R'$ undergoes independent validation:
\begin{itemize}
    \item \textbf{Proposition 1 Monotonicity}: Verified across 100\% of runs ($\min \Delta TT \ge 0.0000$ min, exactly 0 violations). Resequencing never degraded route performance.
    \item \textbf{Hard Feasibility}: 100\% feasibility rate across all runs (no subtours, no duplicated or omitted customers, depot departure and arrival preserved, and cumulative distance constraint $D(R') \le (1+\delta)D(R^0)$ strictly enforced).
\end{itemize}

\subsection{Statistical Methodology and Empirical Framing}\label{sec:statistical-methodology}
\paragraph{Route-Clustered Statistical Inference.}
Standard cell-level pooling treats repeated parameter evaluations on the same route as independent observations, artificially inflating statistical degrees of freedom ($N=48$). To prevent pseudo-replication, statistical inference is conducted using route-clustered paired tests ($N=3$ independent test routes) with Holm--Bonferroni multiplicity correction.

\paragraph{Core Finding and Equivalence Distinction.}
The empirical results reveal clear \emph{actionability dominance}: the actionability score $g_i$ captures available single-vehicle schedule headroom, and actionability-based policies (RA and AB) achieve substantial simulated tardiness reductions, whereas pure risk targeting ($RB$) fails.

Crucially, in comparing RA with AB:
\begin{quote}
\emph{No evidence of a difference between RA and AB was detected.}
\end{quote}
A nonsignificant difference ($p = 0.4825$) does not establish equivalence. Demonstrating statistical equivalence requires a prespecified practical equivalence margin and a formal Two One-Sided Tests (TOST) or confidence-interval equivalence procedure on an expanded cohort. Based on exploratory variance across the test cohort, a prospective power calculation provides a provisional planning estimate of $N \approx 34$ independent routes for future scaled evaluations.

Definitive claims regarding the generalizability of PISR across diverse operational settings and the incremental value of risk weighting are appropriately reserved for Section 5 upon expansion to the broader Amazon Challenge cohort. The present conclusion remains exploratory: actionability drives the operational improvement, while risk weighting has not yet demonstrated incremental benefit over AB in this single-vehicle setting.
